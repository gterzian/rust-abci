...
struct Protocol {
    ...
    network_exit_sender: Option<oneshot::Sender<ExitSignal>>,
}
...
impl Protocol {
    ...
    fn exit(&mut self) {
        ...
        let _ = self
            .network_exit_sender
            .take()
            .expect("To have an network exit sender")
            .send(ExitSignal);
    }
    ...
    A: Application,
{
    ...
    let mut protocol = Protocol {
        ...
        exit_sender,
        exit_receiver,
        network_exit_sender: Some(network_exit_sender),
    };
    thread::spawn(move || {
        protocol.start();
        while protocol.run() {
...
mod tests {
    ...
    fn test_protocol_calls_app() {
        ...
        impl Application for ChecksApp {
            ...
            fn deliver_tx(&mut self, _req: RequestDeliverTx, mut responder: Responder) {
                let mut response = Response::new();
                let res = ResponseDeliverTx::new();
                response.set_deliver_tx(res);
                let _ = responder.respond(response);
